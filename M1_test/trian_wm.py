...
def train(config, train_loader,watermark_loader, model, criterion, optimizer):
    ...
    for input, target, _ in train_loader:
        ...
        for input2, target2, _ in watermark_loader:
                ...
                loss3 = criterion(output2, target2)
                loss2 = 0
                for i in range(output2.size(0)):
                    ...
                    loss2 += (1 - ssim(output, target, multichannel=True))
                    loss2=torch.tensor(loss2)
        loss2=loss2/output2.size(0)
        f_lambda1 = 1
        f_lambda2=0
        f_lambda3=0
        loss = f_lambda1 * loss1 + f_lambda2 * loss2+f_lambda3*loss3
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        ...
        drop_last=False)  # 不能整除的batch是否就不要了
    ...
    for epoch in range(config['epochs']):
        ...
        # train for one epoch
        train_log = train(config, train_loader,watermark_loader, model, criterion, optimizer)
        ...
        if val_log['iou'] > best_iou:
            torch.save(model.state_dict(), 'models/train_wm/model.pth')
